...
def test_start_function_option() -> None:
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        # Create test database files with different filenames
...
def test_list_functions_option(tmp_path: Path) -> None:
    ...
    result = CliRunner().invoke(cli, ["-l", "-i", str(tmp_path)], standalone_mode=False)
...
def test_search_option(tmp_path: Path) -> None:
    ...
    """
    create_temp_source_file(tmp_path, "void pattern_func(void) {}", "pattern.c")
...
def test_no_abbreviate_rte_option(tmp_path: Path) -> None:
    ...
    if output_file.exists():
        # May or may not have abbreviations depending on implementation
        # The key is that the CLI executes successfully
        pass
    ...
    output_file = tmp_path / "call_tree.md"
    if output_file.exists():
        # May or may not have full names depending on implementation
        # The key is that the CLI executes successfully
        pass
...
"""
    ...
    # Create test files for module configuration
    create_temp_source_file(tmp_path, "void test_func(void) { hw_func(); }", "test.c")
...
def test_version_option() -> None:
    ...
    """
    # Test version option
    result = CliRunner().invoke(cli, ["--version"], standalone_mode=False)
