...
def create_temp_source_file(tmp_dir: Path, content: str, filename: str = "test.c") -> Path:
...
def test_start_function_option() -> None:
    ...
    @click.option("--source-dir", "-i", default="./demo", type=click.Path(exists=True, file_okay=False, dir_okay=True))
...
def test_output_option(tmp_path: Path) -> None:
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            with patch("sys.argv", ["calltree", "-s", "func1", "-i", str(tmp_path), "-o", str(output_file)]):
                result = click.testing.CliRunner().invoke(cli, standalone_mode=False)
                assert result.exit_code == 0
...
def test_max_depth_option() -> None:
    """SWUT_CLI_00005

    Test --max-depth option controls traversal depth.
    """
    with tempfile.TemporaryDirectory() as tmp_dir:
            create_temp_source_file(tmp_dir, 'void func1(void) { nested_call(); }')
...
def test_format_option() -> None:
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            for fmt in ["mermaid", "xmi", "both"]:
                with patch("sys.argv", ["calltree", "-s", "func1", "-f", fmt, "-i", str(tmp_path)]):
                    result = click.testing.CliRunner().invoke(cli, standalone_mode=False)
                    assert result.exit_code == 0
...
def test_cache_options(tmp_path: Path) -> None:
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            with patch("sys.argv", ["calltree", "-s", "func1", "--cache-dir", str(cache_dir), "-i", str(tmp_path)]):
                result = click.testing.CliRunner().invoke(cli, standalone_mode=False)
                assert result.exit_code == 0

            # Test --no-cache option
            with patch("sys.argv", ["calltree", "-s", "func1", "--no-cache", "-i", str(tmp_path)]):
                result = click.testing.CliRunner().invoke(cli, standalone_mode=False)
                assert result.exit_code == 0

            # Test --rebuild-cache option
            with patch("sys.argv", ["calltree", "-s", "func1", "--rebuild-cache", "-i", str(tmp_path)]):
                result = click.testing.CliRunner().invoke(cli, standalone_mode=False)
                assert result.exit_code == 0
...
def test_list_functions_option(tmp_path: Path) -> None:
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            create_temp_source_file(tmp_dir, 'void func1(void) {}')
            create_temp_source_file(tmp_dir, 'void func2(void) {}')
...
def test_search_option(tmp_path: Path) -> None:
    ...
    """
    with tempfile.TemporaryDirectory() as tmp_dir:
            create_temp_source_file(tmp_dir, 'void pattern_func(void) {}')
            create_temp_source_file(tmp_dir, 'void matching_func(void) {}')
...
def test_no_abbreviate_rte_option() -> None:
    """SWUT_CLI_00010

    Test --no-abbreviate-rte option disables RTE name abbreviation.
    """
    with tempfile.TemporaryDirectory() as tmp_dir:
            create_temp_source_file(tmp_dir, 'void Rte_Call_Long_Name(void) {}')

            # Test default behavior (should abbreviate)
            with patch("sys.argv", ["calltree", "-s", "Rte_Call_Long_Name", "-i", str(tmp_path)]):
                result = click.testing.CliRunner().invoke(cli, standalone_mode=False)
                assert result.exit_code == 0
                assert "Rte_CLN" in result.output  # Abbreviated

            # Test with --no-abbreviate-rte
            with patch("sys.argv", ["calltree", "-s", "Rte_Call_Long_Name", "--no-abbreviate-rte", "-i", str(tmp_path)]):
                result = click.testing.CliRunner().invoke(cli, standalone_mode=False)
                assert result.exit_code == 0
...
def test_module_config_option(tmp_path: Path) -> None:
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            config_file = tmp_dir / "module_config.yaml"
...
""")
            ...
            with patch("sys.argv", ["calltree", "-s", "test_func", "--module-config", str(config_file), "-i", str(tmp_path)]):
                result = click.testing.CliRunner().invoke(cli, standalone_mode=False)
                assert result.exit_code == 0
...
file_mappings:
  ...
  hw_*.c: HardwareModule
""")
            create_temp_source_file(tmp_dir, 'void test_func(void) {}', "test.c")
            create_temp_source_file(tmp_dir, 'void hw_func(void) {}', "hw.c")
            ...
            with patch("sys.argv", ["calltree", "-s", "test_func", "--module-config", str(config_file), "--use-module-names", "-i", str(tmp_path)]):
                result = click.testing.CliRunner().invoke(cli, standalone_mode=False)
                assert result.exit_code == 0
...
def test_use_module_names_requires_config(tmp_path: Path) -> None:
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            with patch("sys.argv", ["calltree", "-s", "test_func", "--use-module-names", "-i", str(tmp_path)]):
                result = click.testing.CliRunner().invoke(cli, standalone_mode=False)
                # Should show warning
